...
const Features = (props) => {
	...
	const renderFilter = () => (
		<>
			<input placeholder='Search: id, type, name' value={searchValue} onChange={(e) => setSearchValue(e.target.value)} />
			{radioList.map(item => <div key={item.value} className={clsx('filter-radio', {'active': radioValue===item.value})} onClick={() => setRadioValue(item.value)}>{item.label}</div>)}
	...
	const geoJSON = useMemo(() => {
		...
		if(searchValue) {
			data = data.filter(item => checkSubString(item.id) || checkSubString(item.type) || checkSubString(item.properties?.name));
		} 
		if(radioValue === 'route') {
	...
	return (
		<Modal title="Feature Collection" onClose={handleClose} show={show} filter={renderFilter} handleScroll={handleScroll}>
			<div className='list'>
				{geoJSON?.slice(0, dataCount)?.map((item, i) => 
					<div className='card' key={item.id}>
						<div><b>ID:</b>{' '}<span>{item.id}</span></div>
						<div><b>Type:</b>{' '}<span>{item.type}</span></div>
						{item.properties?.name ? <div><b>Name:</b>{' '}<span>{item.properties.name}</span></div> : null}
						<div>
							<b>Properties</b><span className='toggle-btn' onClick={() => handleExpand(propExpand, setPropExpand, i)}>{propExpand.includes(i) ? '-' : '+'}</span>
							{propExpand.includes(i) ? <div className='properties'>
						...
						<div>
							<b>Geometry</b><span className='toggle-btn' onClick={() => handleExpand(geometryExpand, setGeometryExpand, i)}>{geometryExpand.includes(i) ? '-' : '+'}</span>
							{geometryExpand.includes(i) ? <div className='properties'>
								{Object.entries(item.geometry).map(([key, value]) => 
									<div key={key}>
										<div><b>{key}{': '}</b><span>{JSON.stringify(value)}</span></div>
									</div>
								)}
							</div> : null}
						</div>
